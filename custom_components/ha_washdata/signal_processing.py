...
def robust_smooth(power: np.ndarray, timestamps: np.ndarray, time_constant_s: float = 30.0) -> np.ndarray:
    ...
    if len(power) >= 3:
        ...
        stack = np.vstack([p_padded[0:len(power)], p_padded[1:len(power)+1], p_padded[2:]])
...
def resample_uniform(
    ...
    for start_idx, end_idx in zip(start_indices, end_indices):
        ...
        # target_ts = np.arange(grid_start, grid_end, dt_s)
        # np.arange is exclusive of end. We want inclusive if it aligns.
        # But if we want exactly cover duration, we should ensure we span approx same time.
        # Let's use linspace? No, we want fixed dt.
        
        # If we use arange(start, end + epsilon, dt)
...
def resample_adaptive(
    ...
    Target dt = max(median_interval, min_dt).
    This ensures we never upsample (interpolate finer than reality) which creates fake data,
    but we do downsample heavily oversampled data (spikes/noise) to min_dt.
    
    Args:
        timestamps: Raw timestamps (seconds).
        power: Raw power values.
        min_dt: Minimum allowed dt (seconds). We will downsample to this if input is finer.
        max_dt: (Unused currently, ensuring we ignore 'clamp' upsampling advice to avoid fake data)
    ...
    # Logic: Never upsample (fake data). Downsample if too noisy.
    target_dt = max(median_dt, min_dt)
